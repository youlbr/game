...
import questionIMG2 from "../assets/QUESTION2.png";
...
export default class Examen3 extends Phaser.Scene {
  constructor() {
    super("Examen3");
  }
  ...
  preload() {
    ...
    this.load.image("question2",questionIMG2);
  ...
  crear() {
    ...
    const updateText = () => {
      ...
      currentText = this.add.text(this.cameras.main.width / 2, y - 400, examData.tuelvenWords[count].spanish, textConfigEnglish).setOrigin(0.5, 0.5);
      currentText.setColor('#00afef')
  
      let alternatives = examData.tuelvenWords
        .flatMap((ele) => (ele.english !== examData.tuelvenWords[count].english ? [ele.english] : []))
        .slice(0, 5);
      alternatives.push(examData.tuelvenWords[count].english);
      ...
      for (let i = 0; i < alternatives.length; i++) {
        ...
        hitArea.on("pointerdown", () => {
          ...
          if (wordAlternative === examData.tuelvenWords[count].english) {
            this.soundWin.play();
            nextBoton.clear();
